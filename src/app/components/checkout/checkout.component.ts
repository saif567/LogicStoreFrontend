...
export class CheckoutComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //read the user's email address fromm browser storage
    const theEmail = JSON.parse(this.storage.getItem('userEmail'));
    const currentMonth = new Date().getMonth() + 1;
    this.formService.getCreditCardMonth(currentMonth).subscribe(
      data => {
        console.log(`Retrive the Card Month ${JSON.stringify(data)}`);
    ...
    this.formService.getCreditCardYear().subscribe(
      data => {
        console.log(`Retrive the Card Year ${JSON.stringify(data)}`);
  ...
  onSubmit() {
    ...
    this.checkoutService.placeOrder(purchase).subscribe({
        next:reponse=>{
          alert(`Your order has been received\n Order tracking number is: ${reponse.orderTrackingNumber}`);
  ...
  resetCart() {
    ...
    //nevigate back to the product page
  ...
  handleMonthsAndYear() {
    ...
    this.formService.getCreditCardMonth(startMonth).subscribe(
      data => {
        console.log(`After Change ${JSON.stringify(data)}`);
        this.creditCardMonth = data;
      }
    );
  }
